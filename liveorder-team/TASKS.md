...
_Eddy(PM) 관리_
_최종 업데이트: 2026-04-09 (Planner — Task 46 완료 확인 + Task 47 스펙 수립)_
...
- [ ] 관리자 세션 없으면 401
- [ ] 없는 ID → 404
- [ ] stats 5개 필드 반환 (productCount, codeCount, orderCount, totalRevenue, pendingSettlement)
...
**완료 조건:**
- [ ] 페이지네이션 동작 (`page`, `totalPages`, `total`)
- [ ] `?status=PAID` 등 필터 동작
- [ ] 빈 결과도 정상 반환 (404 아님)
...
**완료 조건:**
- [ ] 셀러 기본정보 전체 표시 (이메일, 대표자, 사업자번호, 연락처, 주소, 정산계좌)
- [ ] 사업자등록증 이미지 링크 (있을 때만)
- [ ] 통계 카드 4개 (총 주문수, 총 매출, 활성 상품수, 대기 정산액) 정상 표시
- [ ] 주문 목록 페이지네이션 동작
- [ ] 상태 필터 동작 (전체/PAID/SHIPPING/DELIVERED/REFUNDED)
- [ ] 승인/거부/정지 버튼 동작 + 상태 즉시 반영
...
- [ ] `git add app/api/admin/sellers/ app/admin/sellers/`
- [ ] `git commit -m 'feat: Task 47 — 관리자 셀러 상세 페이지 (47A~47D)'`
