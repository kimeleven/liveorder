...
_Eddy(PM) 관리_
_최종 업데이트: 2026-04-09 (Planner — Task 39 완료 확인 + Task 40 스펙 수립)_
...
### Task 40: 주문 소스 추적 (`source: 'web' | 'kakao'`)

**우선순위:** MEDIUM
**이유:** 셀러가 카카오 경로 주문을 웹 주문과 구분하여 CS/분석에 활용
...
`Order` 모델에 `source` 필드 추가 (`pgTid` 바로 위에 삽입):
```prisma
...
#### 40B: 카카오 진입 페이지에 소스 플래그 저장

**파일 수정:** `app/(buyer)/kakao/[token]/page.tsx`

`sessionStorage.setItem('pendingCode', ...)` 직후, `router.replace('/chat')` 전에 추가:
```typescript
...
#### 40C: PaymentSummary에서 source 전송

**파일 수정:** `components/buyer/cards/PaymentSummary.tsx`

`handlePayment()` 함수 내 try 블록 첫 줄 (PortOne SDK 로드 전)에 추가:
```typescript
const source = sessionStorage.getItem('kakaoSource') === 'true' ? 'kakao' : 'web'
```

`fetch("/api/payments/confirm", ...)` body에 추가:
```typescript
source,  // 'web' | 'kakao'
```
...
#### 40F: 셀러 주문 목록 UI에 카카오 배지 표시

**파일 수정:** `app/seller/orders/page.tsx`

1. `OrderItem` 인터페이스에 `source: string` 추가

2. 주문자명 `TableCell`을 수정:
```tsx
<TableCell>
...
- [ ] `prisma/schema.prisma` — `Order`에 `source` 필드 추가
- [ ] `npx prisma migrate dev --name add_order_source` 실행
- [ ] `app/(buyer)/kakao/[token]/page.tsx` — `kakaoSource` sessionStorage 저장
- [ ] `components/buyer/cards/PaymentSummary.tsx` — `source` 필드 API 전송
- [ ] `app/api/payments/confirm/route.ts` — `source` 필드 저장
- [ ] `app/api/seller/orders/route.ts` — `source: true` select 추가
- [ ] `app/seller/orders/page.tsx` — 카카오 배지 UI
- [ ] git commit + push
