...
_Eddy(PM) 관리_
_최종 업데이트: 2026-04-09 (Dev1 — Task 50 신규: 관리자 대시보드 개선)_
...
- [ ] `dailySales` 배열 반환 (BigInt → number 변환)
- [ ] `recentOrders` 최근 5건 (seller.name 포함)
- [ ] `pendingSellerList` 최근 5건
...
**완료 조건:**
- [ ] 통계 카드 6개 (오늘 매출, 이번달 매출 포함)
- [ ] 매출 차트 + 일별/주별/월별 토글
- [ ] 승인 대기 셀러 테이블 + 클릭 → `/admin/sellers/[id]`
- [ ] 최근 주문 테이블 + 클릭 → `/admin/orders/[id]`
- [ ] 에러 배너 표시
- [ ] `AdminShell` 래핑 유지
