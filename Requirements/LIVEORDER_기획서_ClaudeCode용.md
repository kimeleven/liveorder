...
| 날짜 | 버전 | 변경 내용 |
|------|------|-----------|
| 2026-04-10 | v3.6 | Task 61 완료 (관리자 셀러 목록 서버사이드 페이지네이션 + 검색 + CSV 내보내기). Task 62 스펙 수립: 관리자 주문 검색(`?q=` 구매자명/전화번호) + 날짜 범위 필터(`?from=`, `?to=`) + `GET /api/admin/orders/export` CSV 내보내기 + UI 개선. |
...
| 이용약관 + 판매자 약관 동의 | 회원가입 폼 체크박스 | ✅ (Task 45D) |
...
| 주문 목록 Skeleton 로딩 | ✅ |
| 대시보드 에러 처리 | ✅ |
| 운송장 일괄 업로드 (CSV) | ✅ (Task 43) |
| 매출 통계 (주별/월별) | ✅ (Task 44) |
| 주문 목록 30초 자동갱신 | ✅ (Task 44) |
